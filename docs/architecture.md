...
**OM in model text:** Vagdhenu's frontend maps `ॐ` → SLP1 `oM` → Kannada ಒಂ (short-o), which is out-of-distribution for the chant corpus — the model can swallow it entirely (the missing Gāyatrī ॐ). The shard builders spell the model text `ओं` (U+0913 U+0902, long-ō — the traditional chant spelling); the display padas stay `ॐ`.
